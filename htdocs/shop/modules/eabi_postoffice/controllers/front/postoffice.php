 ...
 * @author Matis
 */
class Eabi_PostofficePostofficeModuleFrontController extends ModuleFrontController {
    public $ssl = true;
    ...
    public function initContent() {
        try {
            if (isset($_POST) && count($_POST)) {
                ...
                $post = $_POST;
                $url = str_replace('/'.Language::getIsoById(Context::getContext()->language->id).'/', '/', $this->context->link->getModuleLink('eabi_postoffice', 'postoffice', array(), true));
                ...
                /* @var $handlerModel Eabi_Postoffice */
                $handlerModel = Module::getInstanceByName('eabi_postoffice');
                
                if (!Validate::isLoadedObject($handlerModel)) {
                    throw new Exception(sprintf('Handler model not found for code %s', 'eabi_postoffice'));
                }
                ...
                if (isset($shippingModel->text) && $shippingModel->text) {
                    $html .= '<p class="eabi_shipping_method_title">' . htmlspecialchars($shippingModel->text) . '</p>';
                } else {
                    ...
                    $html .= '<p class="eabi_shipping_method_title">' . htmlspecialchars($finalTitle) . '</p>';
                }
