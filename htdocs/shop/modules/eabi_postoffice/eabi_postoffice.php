 ...
 * @author Matis
 */
class Eabi_Postoffice extends Module {
    ...
    public function __construct() {
        ...
        $this->name = 'eabi_postoffice';
        $this->version = '0.5';
        $this->ps_versions_compliancy = array('min' => '1.5', 'max' => '1.7');
        parent::__construct();
        $this->displayName = $this->l('E-Abi Generic Office plugin');
     ...
     * <ul>
         <li>Creates database table <code>eabi_carriermodule</code> for holding list of carriers using this class</li>
         <li>Creates database table <code>eabi_postoffice</code> for holding pickup points for the carrier modules</li>
         <li>Creates database table <code>eabi_cart_shipping</code> for holding information which shipping address in the cart has selected which parcel terminal</li>
    ...
    public function getPostOffices($code, $groupId = null, $officeId = null, $addressId = null) {
        ...
        $db = Db::getInstance();
        $sql = "SELECT * FROM " . _DB_PREFIX_ . "eabi_postoffice "
    ...
    public function getPostOfficeGroups($code, $addressId = null) {
        $db = Db::getInstance();
        $sql = "SELECT DISTINCT group_id, group_name, group_sort FROM "._DB_PREFIX_."eabi_postoffice "
                ." WHERE remote_module_name = '{$db->escape($code)}'"
    ...
    /**
     * <p>Once the user selects the actual office, an AJAX callback is performed and this one inserts the selected office to the database <code>eabi_cart_shipping</code>
	...
	`remote_place_id` int(11) unsigned NOT NULL,
         * ps_eabi_cart_shipping
         */
        $id_cart = Context::getContext()->cart->id;
        $db = Db::getInstance();
        $sql = "SELECT * FROM ". _DB_PREFIX_ ."eabi_cart_shipping ";
...
//        $sql .= " AND id_address = '{$db->escape($addressId)}' ";
        ...
        if (count($oldInserts)) {
            //update
//            $db->update('eabi_cart_shipping', $data, " id_cart = '{$db->escape($id_cart)}' AND id_address = '{$db->escape($addressId)}' ");
            $db->update('eabi_cart_shipping', $data, " id_cart = '{$db->escape($id_cart)}' ");
        } else {
            //insert
            $db->insert('eabi_cart_shipping', $data);
        }
    ...
    /**
     * <p>Registers carrier to <code>eabi_carriermodule</code> table as:</p>
     * <ul>
    ...
    public function addCarrierModule($code, $class, $trackingUrl = '') {
        ...
        /*
         * 
        `id_eabi_carriermodule` int(11) unsigned NOT NULL auto_increment,
        ...
        $res = $db->getRow('select * from ' . _DB_PREFIX_ . 'eabi_carriermodule where carrier_code = \''.$db->escape($code)."'");
        if (!$res) {
            ...
            $res = $db->insert('eabi_carriermodule', $data);
    ...
    /**
     * <p>Removes carrier from the system, only <code>eabi_carriermodule</code> entry is kept so when reinstalling carrier, then selected parcel terminal names would be restored</p>
     * @param string $code
     * @return boolean
     */
    public function removeCarrierModule($code) {
        ...
        $res = $db->delete('eabi_postoffice', 'remote_module_id = \''.$db->escape($id_carrier).'\'');
        if (!$res) {
            return false;
        }
    ...
    public function hookActionCarrierProcess($params) {
        ...
        $id_address = $this->_getIdAddress($cart->id_address_delivery);
...
//            echo '<pre>'.htmlspecialchars(print_r($v, true)).'</pre>';
    ...
    /**
     * <p>Fix display bug right after login</p>
     * <p>select menu of pickup points not displayed after entering customer address and then logging in instead</p>
     * @param int $origIdAddress default cart->id_address_delivery
     * @return int
     */
    protected function _getIdAddress($origIdAddress) {
        ...
        foreach ($products as $product) {
            if (!$product['is_virtual']) {
                if ($product['id_address_delivery'] != $origIdAddress) {
                    $origIdAddress = $product['id_address_delivery'];
                }
                break;
            }
        }

        //end fix display bug right after login
        return $origIdAddress;
    }
    ...
    public function displayExtraCarrier($code, $params, $shouldHide = false) {
        ...
        //fix display bug right after login.
        //select menu of pickup points not displayed after entering customer address and then logging in instead
        $params['id_address_delivery'] = $this->_getIdAddress($params['id_address_delivery']);

        //end fix display bug right after login
        
        
        $implodedJs = 'input[name=delivery_option\\\[' . $params['id_address_delivery'] . '\\\]][value="' . $prestaCarrierValue->id . ',"]:visible';
        $jsHideSelector = '.parent()';
        if (substr(_PS_VERSION_, 0, 3) == "1.6") {
            $jsHideSelector = ".parentsUntil('div.delivery_option')";
        }
...
//       <![CDATA[
    (function() {
        ...
        jQuery.each(implodedJs, function(i, val) {
            jQuery(val){$jsHideSelector}.hide();
        });
...
EOT;
        ...
        $list = '<div id="eabi_carrier_' . $code . '" style="display:inline-block;"></div>';
   ...
   if (!window.eabi_carrier_checked) {
        jQuery('#id_carrier_{$code}').click();
        window.eabi_carrier_checked = true;
   }
EOT;
        if (substr(_PS_VERSION_, 0, 3) == "1.6") {
...
EOT;
        ...
        $eabiCarrierValue = $prestaCarrierValue->id . ',';
        ...
        $smarty->assign(array(
            'eabi_carrier' => $arr,
            'eabi_ERROR_MESSAGE' => $errorMessage,
            'eabi_url' => str_replace('/'.Language::getIsoById(Context::getContext()->language->id).'/', '/', $this->context->link->getModuleLink($this->name, 'postoffice', array(), true)),
            'eabi_divId' => 'eabi_carrier_' . $code,
            'eabi_carrierId' => $code . '\\\\,',
            'eabi_carrierCode' => $code,
            'eabi_addressId' => $params['id_address_delivery'],
            'eabi_carrierValue' => $containsSelection ? $eabiCarrierValue : '',
            'eabi_extraJs' => $isSelected ? $extraJs : '',
            'eabi_priceDisplay' => $carrier_tax == 0,
        ));
...
//                $data = $carrierInstance->displayInfoByCart($order->id_cart);
    ...
    protected function _getSingleTerminal($code) {
        $db = Db::getInstance();
        $sql = 'SELECT count(id_eabi_postoffice) as cnt FROM `' . _DB_PREFIX_ . 'eabi_postoffice` where `remote_module_name` = \''.$db->escape($code).'\' ';
        $res = $db->executeS($sql);
        if (is_array($res) && count($res) && $res[0]['cnt'] == '1') {
            //fetch
            $sql = 'SELECT * FROM `' . _DB_PREFIX_ . 'eabi_postoffice` where `remote_module_name` = \''.$db->escape($code).'\' LIMIT 1';
    ...
    protected function _verifySelectedTerminal($code, $terminal) {
        $db = Db::getInstance();
        $sql = 'SELECT * FROM `' . _DB_PREFIX_ . 'eabi_postoffice` where `remote_module_name` = \''.$db->escape($code).'\' and remote_place_id = \''.$db->escape($terminal).'\' LIMIT 1';
        $res = $db->executeS($sql);
    ...
    public function idFromCode($code) {
        if (is_null(self::$_carrierModules)) {
            $db = Db::getInstance();
            $sql = 'SELECT * FROM `' . _DB_PREFIX_ . 'eabi_carriermodule`';
            $res = $db->executeS($sql);
            self::$_carrierModules = array();
            foreach ($res as $r) {
                self::$_carrierModules[$r['carrier_code']] = $r['id_eabi_carriermodule'];
            }
        }
    ...
    public function refresh($code, $byPassTimeCheck = false) {
        ...
        if ($lastUpdated + ($updateInterval * 60) < $date || $byPassTimeCheck) {
            ...
            //sql query....
            $oldDataCollection = $db->executeS("select * from " . _DB_PREFIX_ . "eabi_postoffice where remote_module_name = '" . $db->escape($code) . "'");
            $groups = array();
            ...
            if (!is_array($officeList)) {
                ...
                foreach ($oldData as $placeId => $oldDataElement) {
                    if (!isset($processedPlaceIds[(string) $placeId])) {
                        ...
                        $db->execute("delete from " . _DB_PREFIX_ . "eabi_postoffice where id_eabi_postoffice = " . $db->escape($oldDataElement['id_eabi_postoffice']));
                    } else {
                        //save OldDataElement
                        if (!isset($oldDataElement['id_eabi_postoffice'])) {
                            //insert
                            $dataToInsert = array();
                            ...
                            $db->execute("insert into " . _DB_PREFIX_ . "eabi_postoffice (" . implode(',', array_keys($dataToInsert)) . ") VALUES (" . implode(',', $dataToInsert) . ");");
                            ...
                            $db->execute("update " . _DB_PREFIX_ . "eabi_postoffice set " . implode(', ', $dataToInsert) . " where id_eabi_postoffice = " . $db->escape($oldDataElement['id_eabi_postoffice']));
//                            exit;
                        }
                    }
                }
    ...
    protected function fromOfficeElement($officeElement, $moduleCode, $oldData = null) {
        ...
        $newData = array();
        if (is_array($oldData)) {
            if (isset($oldData['id_eabi_postoffice'])) {
                $newData['id_eabi_postoffice'] = $oldData['id_eabi_postoffice'];
        ...
        if ($moduleCode != '') {
            ...
            $remoteModule = $db->executeS("select * from ". _DB_PREFIX_. "eabi_carriermodule where carrier_code = '".$db->escape($moduleCode)."'");
            
            if (!count($remoteModule)) {
                throw new Exception('Carrier could not be detected');
            }
            $newData['remote_module_id'] = $remoteModule[0]['id_eabi_carriermodule'];
    ...
    protected function _verifyCode($code) {
        $db = Db::getInstance();
        $sql = 'SELECT * FROM `' . _DB_PREFIX_ . 'eabi_carriermodule` where `carrier_code` = \''.$db->escape($code).'\' LIMIT 1';
        $res = $db->executeS($sql);
        if (!$res) {
            return false;
        }
    ...
    private function _dropTables() {
		$sql = 'DROP TABLE
			`'._DB_PREFIX_.'eabi_carriermodule`,
			`'._DB_PREFIX_.'eabi_postoffice`,
			`'._DB_PREFIX_.'eabi_cart_shipping`
			';
    ...
    private function _createSelectedShippingOptionInCartTable() {
        $sql = 'CREATE TABLE IF NOT EXISTS `' . _DB_PREFIX_ . 'eabi_cart_shipping`(
	`id_eabi_cart_shipping` int(11) unsigned NOT NULL auto_increment,
	`id_cart` int(11) unsigned NOT NULL,
	`id_address` int(11) unsigned NOT NULL,
	`remote_module_id` int(11) unsigned NOT NULL,
	`remote_place_id` int(11) unsigned NOT NULL,
	PRIMARY KEY (`id_eabi_cart_shipping`)
	) ENGINE=' . _MYSQL_ENGINE_ . ' DEFAULT CHARSET=utf8';
        $result = Db::getInstance()->execute($sql);
        if ($result === false) {
            return false;
        }
        $indexSql = "ALTER TABLE `"._DB_PREFIX_."eabi_cart_shipping` ADD INDEX ( `id_cart` , `id_address` ) ;";
        $indexSqlResult = Db::getInstance()->execute($indexSql);
        return $indexSqlResult;
    }

    private function _createCarrierModuleTable() {
        $sql = 'CREATE TABLE IF NOT EXISTS `' . _DB_PREFIX_ . 'eabi_carriermodule`(
        `id_eabi_carriermodule` int(11) unsigned NOT NULL auto_increment,
        `carrier_code` varchar(255) NOT NULL,
        `class_name` varchar(255) NOT NULL,
        `update_time` datetime NULL,
	PRIMARY KEY (`id_eabi_carriermodule`)
	) ENGINE=' . _MYSQL_ENGINE_ . ' DEFAULT CHARSET=utf8';
        return Db::getInstance()->execute($sql);
    ...
    private function _createPostofficeTable() {
        $sql = 'CREATE TABLE IF NOT EXISTS `' . _DB_PREFIX_ . 'eabi_postoffice`(
        `id_eabi_postoffice` int(11) unsigned NOT NULL auto_increment,
        `remote_module_id` int(11) unsigned NOT NULL,
	...
	PRIMARY KEY (`id_eabi_postoffice`)
	) ENGINE=' . _MYSQL_ENGINE_ . ' DEFAULT CHARSET=utf8';
        return Db::getInstance()->execute($sql);
    ...
    private function _createIndexes() {
        $sql = '    ALTER TABLE `' . _DB_PREFIX_ . 'eabi_postoffice` ADD UNIQUE (
		`remote_module_id`,
		`remote_place_id`
	);
        ';
